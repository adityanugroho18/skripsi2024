...
class TransaksiKas(models.Model):
    ...
    class Meta:
        db_table = 'transaksi_kas'  # Specify the desired table name

class TransaksiKasDetail(models.Model):
    kode_transaksi = models.ForeignKey(TransaksiKas, on_delete=models.CASCADE)
    kode_lawan = models.ForeignKey(KodeAkunModel.KodeAkun, on_delete=models.CASCADE)
    total = models.DecimalField(max_digits=13, decimal_places=2)
    keterangan = models.TextField(null=True)

    class Meta:
        db_table = 'transaksi_kas_detail'  # Specify the desired table name