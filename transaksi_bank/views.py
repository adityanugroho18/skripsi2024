...
from .models import TransaksiBank, TransaksiBankDetail
...
create_view = 'bank/transaksi/create.html'
edit_view = 'bank/transaksi/edit.html'
detail_view = 'bank/transaksi/detail.html'

def index(request):
    context['title'] = 'List Transaksi Bank'
    query = request.GET.get('q')
    context['query'] = query

    if query:
        bank_list = TransaksiBank.objects.filter(
            Q(kode_transaksi__icontains=query) | Q(tipe__icontains=query) | Q(total__icontains=query)
        ).select_related('kode_akun')
    else:
        bank_list = TransaksiBank.objects.all().select_related('kode_akun')

    pagination_data = pagination(request, bank_list, per_page=10)
    ...
    return render(request, 'bank/transaksi/index.html', context)

def create(request):
    context['title'] = 'Tambah Transaksi Bank'
    context['tipe'] = [TransaksiBank.MASUK, TransaksiBank.KELUAR]
    context['kode_akun'] = KodeAkun.objects.filter(
        Q(nama__icontains='Bank')
...
def store(request):
    if request.method == 'POST':
        try:
            ...
            kode_transaksi = generate_transaction_code('bank', tipe)
            d_nominal = form.getlist('d_nominal[]')
            d_ket = form.getlist('d_ket[]')
            d_kode_lawan = form.getlist('d_kode_lawan[]')

            with transaction.atomic():
                # Store to transaksi_bank table
                bank = TransaksiBank.objects.create(
                ...
                for i, val in enumerate(d_nominal):
                    # Store to transaksi_bank_detail table
                    d_nominal_val = clear_currency_format(val)
                    d_ket_val = d_ket[i]
                    d_kode_lawan_val = d_kode_lawan[i]

                    detail = TransaksiBankDetail.objects.create(
                ...
                record_activity(
                    request,
                    f"Menambahkan data transaksi Bank dengan kode '{kode_transaksi}' dan total '{total_formated}'",
                    kategori=LogActivity.TRANSAKSI,
                    jenis_transaksi=LogActivityDetail.BANK,
                    tipe=LogActivityDetail.CREATE
                )

                messages.success(request, 'Berhasil menyimpan data')
                del context['fieldValues']
                return redirect('/bank/transaksi/')
        ...
        return redirect('/bank/transaksi/create')
    return redirect('/bank/transaksi/create')

def detail(request, kode_transaksi):
    context['title'] = 'Rincian Transaksi Bank'
    context['tipe'] = [TransaksiBank.MASUK, TransaksiBank.KELUAR]
    context['kode_akun'] = KodeAkun.objects.filter(
        Q(nama__icontains='Bank')
    ).select_related('kode_induk')
    ...
    context['transaksi'] = TransaksiBank.objects.filter(kode_transaksi=kode_transaksi).select_related('kode_akun').first()
    detail_list = TransaksiBankDetail.objects.filter(kode_transaksi=kode_transaksi).select_related('kode_lawan')
...
def edit(request, kode_transaksi:str):
    context['title'] = 'Edit Transaksi Bank'
    context['tipe'] = [TransaksiBank.MASUK, TransaksiBank.KELUAR]
    context['kode_akun'] = KodeAkun.objects.filter(
        Q(nama__icontains='Bank')
    ).select_related('kode_induk')
    ...
    context['transaksi'] = get_object_or_404(TransaksiBank, kode_transaksi=kode_transaksi)
    context['detail'] = TransaksiBankDetail.objects.filter(kode_transaksi_id=kode_transaksi)
...
def update(request, kode_transaksi):
    if request.method == 'POST':
        try:
            ...
            current = TransaksiBank.objects.get(kode_transaksi=kode_transaksi)
            current_detail = TransaksiBankDetail.objects.filter(kode_transaksi_id=kode_transaksi)

            with transaction.atomic():
                # Update transaksi_bank table
                ...
                for item in current_detail:
                    if str(item.id) not in d_id:
                        detail_old = TransaksiBankDetail.objects.filter(id=item.id).first()
                        if detail_old:
                            detail_old.delete()
                ...
                for i, detail_id in enumerate(d_id):
                    ...
                    if detail_id == 0:
                        ...
                        # Update transaksi_bank_detail table if have a changes
                        detail = TransaksiBankDetail.objects.filter(id=detail_id).first()
                        if detail:
                ...
                record_activity(
                    request,
                    f"Memperbarui data transaksi Bank dengan kode '{kode_transaksi}' dan total '{total_formated}'",
                    kategori=LogActivity.TRANSAKSI,
                    jenis_transaksi=LogActivityDetail.BANK,
                    tipe=LogActivityDetail.CREATE
                )
        ...
        return redirect('/bank/transaksi/edit/'+kode_transaksi)
    return redirect('/bank/transaksi/edit/'+kode_transaksi)

def delete(request, kode_transaksi):
    try:
        # Retrieve the transaksi Bank object to be deleted
        bank = get_object_or_404(TransaksiBank, kode_transaksi=kode_transaksi)
        ...
        if request.method == 'POST':
            with transaction.atomic():
                ...
                # Record log ativity
                record_activity(request, 'Menghapus data transaksi Bank', kategori=LogActivity.TRANSAKSI)
    ...
    except TransaksiBank.DoesNotExist:
        messages.warning(request, 'Transaksi Bank tidak ditemukan.')
        return redirect('/bank/transaksi')
            ...
            if kode_akun:
                ...
                if dari and sampai:
                    try:
                        ...
                        dari = None
                        sampai = None
                laporan_list = TransaksiBankDetail.objects.filter(
...
def export_to_pdf(request):
    ...
    if kode_akun and dari and sampai:
        laporan = TransaksiBankDetail.objects.filter(
            kode_transaksi__kode_akun=kode_akun,
            kode_transaksi__tanggal__range=(dari, sampai)
        ).select_related('kode_transaksi')
    elif kode_akun:
        ...
        laporan = TransaksiBankDetail.objects.all().select_related('kode_transaksi')
...
def export_to_excel(request):
    ...
    dari = request.GET.get('dari', None)
    sampai = request.GET.get('sampai', None)
    filename = f"Laporan Bank Periode {dari} sd {sampai}.xlsx"
